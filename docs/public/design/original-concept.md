...
> For current API docs, see the [guides](/guides) and the [Markdown Spec Reference](/reference/text-dsl).
...
scene({
	...
	sceneFn: async ([user]) => {
		...
		const newUsername = 'Scene Da'
		form = user.getByTestId('main-form')

		form.getByTestId('input-username').fill(newUsername)
		await form.submit()
