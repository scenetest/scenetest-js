...
- `dashboard.tsx` — exports **`Dashboard`** (the whole app, a plain Preact component `{ transport, theme?, base? }`). Both hosts are Preact and render `<Dashboard>` directly — there's no imperative mount wrapper. Renders into the light DOM under a `.scenetest-dashboard` root (theme applied as inline `--st-*` custom properties); routes on `location.pathname` (`/__scenetest` Home · `/__scenetest/runner` Runner · `/__scenetest/waterfall` Waterfall). **`useDashboardStore(transport)` builds the four collections once** (`scenesProjection`/`assertionsProjection`/`actionsProjection`/`runsProjection` via `createCollection` + `runCollectionOptions`, sharing one `createRunSource`) and passes them down; the root only tracks connection liveness. Each view reads the tables it needs via `useLiveQuery`, so re-renders are scoped. The Waterfall view (`WaterfallHost`) is now just a Preact subtree under `.waterfall-host` (the stylesheet scopes its widget styles there) reading the same store. Dev + cloud render this same component; only the transport differs. Replaced `vite-plugin/src/analyze-app.ts` (deleted).
...
- `select-waterfall.ts` — `selectWaterfall(slice)`: takes a latest-run `RunSlice` and derives the Waterfall's `DashboardState` (lanes by actor, attributed assertions, run rollup). (Replaced the old `applyEvent` event-fold.) + `completedSceneCount`, `initialState`.
