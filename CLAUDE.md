...
The views are **TSX/JSX** (the package has a build step, so no htm), and read the store reactively with **`useLiveQuery`** (`use-live-query.ts`) — no hand-rolled `subscribeChanges`. Navigation is real `<a>` links, delegated on the app root (no preact-iso: its document-level delegation is heavier than needed for a single mounted root). It's a **plain light-DOM app** — no shadow root; CSS ships as `style.css` scoped under `.scenetest-dashboard`, imported by the host (`import '@scenetest/dashboard/style.css'`). (The observer panel in `@scenetest/checks/panel` still uses its own shadow root; the dashboard does not.)

- `dashboard.tsx` — exports **`Dashboard`** (the whole app, a plain Preact component) and **`mountDashboard(element, { transport, theme?, base? })`**, thin sugar that renders `<Dashboard>` into `element` for non-Preact hosts. Renders into the light DOM under a `.scenetest-dashboard` root (theme applied as inline `--st-*` custom properties); routes on `location.pathname` (`/__scenetest` Home · `/__scenetest/runner` Runner · `/__scenetest/waterfall` Waterfall). **`useDashboardStore(transport)` builds the four collections once** (`scenesProjection`/`assertionsProjection`/`actionsProjection`/`runsProjection` via `createCollection` + `runCollectionOptions`, sharing one `createRunSource`) and passes them down; the root only tracks connection liveness. Each view reads the tables it needs via `useLiveQuery`, so re-renders are scoped. The Waterfall view (`WaterfallHost`) is now just a Preact subtree under `.waterfall-host` (the stylesheet scopes its widget styles there) reading the same store. Dev + cloud mount this same app; only the transport differs. Replaced `vite-plugin/src/analyze-app.ts` (deleted).
- `runner.tsx` — the Runner view: `RunnerView` + `Tree`/`ListPane`/`Detail`/`SpecSnippet`/`CopyButton`. Live reads the collections via `useLiveQuery` → `selectSnapshot`; past runs map a `/__scenetest/runs/:id` report. (Text filter + section grouping stay reactive selectors over the live rows — groupBy yields aggregates, not bucketed lists.)
- `select-runner.ts` — Runner selectors: `selectSnapshot(scenes, assertions, actions, runs)` derives per-scene assertions/timeline from the collection rows via `attributeToScene`; `mapReportToSnapshot()` adapts a past-run JSON report into the same shape.
- `app.tsx` — `Waterfall` view component `{ state, send }` + `Header`/`SceneCard` + `sceneSummary()`. A pure view — `WaterfallHost` passes `state` (from `selectWaterfall`).
- `select-waterfall.ts` — `selectWaterfall(scenes, assertions, actions, runs)`: derives the Waterfall's `DashboardState` (lanes by actor, attributed assertions, run rollup) from the collection rows. (Replaced the old `applyEvent` event-fold.) + `completedSceneCount`, `initialState`.
