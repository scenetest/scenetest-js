...
const Home: Component = () => {
  // Demo assertions that fire on page load
  onMount(() => {
    should('docs page loaded', true)
    should('observer panel initialized', typeof window !== 'undefined' && !!(window as any).__scenetest_panel)
  })
  ...
  return (
    <article>
      ...
      <p class="subtitle">
        ...
        product and your mental model – not your tests.
      </p>
      ...
      <pre><code class="language-typescript">{`// in your UI component
  ...
  assert(
    ...
    () => postsCollection.get(id)
      ...
      <p>
        ...
        submit the form, see the success message.</em> Scenes are about orchestration – driving the
      ...
      <pre><code class="language-typescript">{`// in scenes/profile-update.spec.ts
scene('User updates their profile', async ({ cast }) => {
  ...
  // That's it. All the inline assertions fired automatically.
      ...
      <pre><code class="language-typescript">{`import { should, failed, assert, useTestEffect } from '@scenetest/react'
...
export function ProfileForm({ userId }) {
  ...
  return !profile ? <CreateProfileForm />
    : <EditProfileForm profile={profile} />
      ...
      <pre><code class="language-typescript">{`// ❌ "reverse server action" approach
...
expect({
  ...
  updated_string: localDeck.updated_string,
      ...
      <pre><code class="language-typescript">{`// ✅ server assertion triggered after a mutation
onSettled: (data) => {
  assert(
    ...
    () => ({
      ...
      <p>
        ...
        same assertions in new places, or b) create helper functions that DRY this testing logic –
      ...
      <p>
        Scenetest avoids this repetitiveness without the abstraction – by putting the assertions in with
      ...
      <p>
        <em>
          ...
          {' '}(<a href="https://twitter.com/mhsnook">or tw</a>) and let me know what you think. — M
        </em>
      </p>

      <footer>
        <a href="/faq.html">FAQ</a> — comparisons with page.evaluate, Vitest, etc.
        <p class="copyright">
          <span class="footer-logo">🎬</span> © m snook 2026 •{' '}
