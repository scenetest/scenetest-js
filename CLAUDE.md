...
- `mount.ts` — `mountDashboard(element, { transport, theme? })`: attaches a shadow root, injects styles, renders the Waterfall widget, returns `{ unmount() }`
- `console.ts` — `mountConsole(element, { transport, theme?, base? })`: the **unified console** (design phase 3). Owns a shadow root + namespaced `console-styles.ts`, routes on `location.pathname` (`/__scenetest` Home · `/__scenetest/runner` Runner · `/__scenetest/dashboard` Waterfall). The Waterfall tab mounts the existing `Dashboard` widget via `mountDashboard` into a **nested** shadow root (style-isolated). Dev + cloud mount this same app; only the transport differs. Replaced `vite-plugin/src/analyze-app.ts` (deleted).
- `runner.ts` — the Runner view (htm): `RunnerView` + `Tree`/`ListPane`/`Detail`/`SpecSnippet`/`CopyButton` + plain-text formatters, ported from the old `analyze-app`. Live data folds the collection **projections** + `attributeToScene`; past runs map a `/__scenetest/runs/:id` report.
- `runner-store.ts` — the Runner read model: `createRunnerStore()` folds the live stream through `scenesProjection`/`assertionsProjection` + `attributeToScene` (no `@tanstack/db` at runtime — the projections are pure) into per-scene assertions/timeline; `mapReportToSnapshot()` adapts a past-run JSON report into the same shape.
- `app.ts` — Preact components (htm tagged templates): `Dashboard` root (the Waterfall — folds the event stream via the store, drives it from the transport), `Header`, `SceneCard`; `sceneSummary()` copy-failures text
- `store.ts` — pure, DOM-free event folding for the Waterfall: `applyEvent()`, `foldEvents()`, `initialState()` — the original inline dashboard's `handleEvent` made immutable
...
- `styles.ts` — Waterfall widget shadow-root CSS; `console-styles.ts` — console chrome CSS (separate shadow root). Theming surface is `--st-bg`/`--st-accent`/`--st-font`/`--st-font-size` only
- `collections/` — read-only TanStack DB read model over the run stream, subpath export `./collections`. Purpose is **live queries** (filter/aggregate/sort) and a shared read model for the cloud (a Durable Object's WebSocket feed → a DB collection), *not* widget reactivity — the widget is already reactive via `store.ts`. **Multi-run:** rows are partitioned by `runId` (the `run:start` timestamp), spanning a whole PR's history; a new `run:start` opens a new partition and does **not** truncate. `createRunSource(transport)` wraps the transport as one shared fan-out stream (buffers the whole session's events for late-attaching collections); `runCollectionOptions({ source, projection })` returns a `CollectionConfig` the consumer passes to **their own** `createCollection` (so the collection is built by the same `@tanstack/db` their `useLiveQuery` uses — single instance for joins). Many collections share one source = one connection, many tables. Projections (`scenesProjection`, `assertionsProjection`, `runsProjection` — the run picker / rollups / flaky surface) are the sole writers, folding `RunEvent`s into a tiny `RowOp` vocabulary (`insert`/`update`/`delete`/`reset`), so they're testable without TanStack DB; client mutations throw. `@tanstack/db` is a **type-only optional peer dependency** — the subpath only `import type`s `CollectionConfig`, nothing imports it at runtime, so the widget entry stays free of it. History/ordering/dedup stay the transport's contract. See `docs/public/design/unified-console.md` for the target: one mountable app (Home/Runner/Waterfall views) over this read model, dev + cloud, organized by PR. **The dev Runner view (`runner-store.ts`) folds the same projections directly into Preact state for reactivity — it does not use `@tanstack/db`, which stays the cloud/live-query path. `attributeToScene` (in `projections.ts`) joins assertions/actions to scenes: stamped scene id when present, else actor + time-window (#215 / 2a).**

The dashboard UI extracted from the Vite plugin's inline HTML string so dev and scenetest-cloud mount the same widget; the dev/cloud difference is confined to the transport adapter (dev: fetch+SSE; cloud: fetch+WebSocket). The dev console is a **real Vite app** — `packages/vite-plugin/app/` (a ~10-line shell: `mountDashboard(root, { transport: createDevTransport() })`) — built by the plugin's `build` (`tsc && vite build -c app/vite.config.ts`) to `packages/vite-plugin/dist-app/` and served as **static files** at `/__scenetest/dashboard` by the middleware. This replaced the old raw-ESM serving (importmap + `/__scenetest/widget/*` + vendored-module routes): because the shell is a normal Vite build, `@tanstack/db` and friends bundle natively — no hand-rolled esbuild. Same shell scenetest-cloud wraps with a different transport. The shell now mounts `mountConsole` (Home/Runner/Waterfall), served at all view routes (`/__scenetest`, `/__scenetest/runner`, `/__scenetest/dashboard`); the old `analyze-app.ts` inline string and `/__scenetest/vendor/*` routes are **deleted**. The remaining unified-console work is the cloud transport + the PR-history report loader (design phases 2 & 4).
