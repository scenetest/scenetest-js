...
export function transformAssertions(code: string, options: TransformOptions = {}): TransformResult | null {
  ...
  traverse(ast, {
    CallExpression(path: NodePath<t.CallExpression>) {
      ...
      if (args.length < 2) {
        console.warn(`[vite-plugin-scenecheck] serverCheck() requires at least (title, serverFn) at ${filename}:${path.node.loc?.start.line}`)
        return
      }
      ...
      // Replace the serverCheck() call with the RPC call
