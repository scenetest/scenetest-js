...
**`seeToast` asserts a new toast.** It waited for a matching element to appear and then to disappear, which passed on a stale toast expiring and failed on a correct toast that hovering kept on screen. It now waits for a toast that appeared after the last interaction, and returns as soon as that toast appears.
...
* **Breaking — `seeToast` asserts that a new toast appeared, and no longer waits for the toast to dismiss.** Each step claims one toast element, and a claimed element never satisfies a later step. So `click save; seeToast ok; click publish; seeToast ok` needs two toasts, where before the save toast's tail could satisfy both — silently under-testing a `publish` that stopped toasting. Node identity is the test, so a repeated identical message still reads as new. ([#246](https://github.com/scenetest/scenetest-js/issues/246))
* Dismissal no longer decides the assertion. A toast library that pauses on hover (sonner does, correctly) can hold a toast on screen for as long as the runner happens to leave the pointer over the toaster, which failed scenes on CI that passed locally; `duration: Infinity` toasts never passed at all. A scene that needs the toast gone — because it covers the next click — now says `notSee` after it.
* **Breaking — a toast standing before an interaction no longer counts.** Every interaction (`click`, `typeInto`, `check`, `select`, `pressKey`, `do`, and the navigation actions) claims what is on screen when it starts, so `seeToast` only accepts a toast that appeared after it. Before, a toast left over from page load or an earlier action could satisfy the first `seeToast` in a scene. Assertions, `wait` and scope moves claim nothing, so `click save; wait 100; seeToast ok` still works; a scene that asserts a toast across a *later* interaction now fails, and says why.
