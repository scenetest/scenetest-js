...
- `collections/` — read-only TanStack DB read model over the run stream, subpath export `./collections`. Purpose is **live queries** (filter/aggregate/sort) and a shared read model for the cloud (a Durable Object's WebSocket feed → a DB collection), *not* widget reactivity — the widget is already reactive via `store.ts`. `createRunSource(transport)` wraps the transport as one shared fan-out stream (buffers the current run for late-attaching collections, resets on `run:start`); `runCollectionOptions({ source, projection })` returns a `CollectionConfig` the consumer passes to **their own** `createCollection` (so the collection is built by the same `@tanstack/db` their `useLiveQuery` uses — single instance for joins). Many collections share one source = one connection, many tables. Projections (`scenesProjection`, `assertionsProjection`) are the sole writers, folding `RunEvent`s into a tiny `RowOp` vocabulary (`insert`/`update`/`delete`/`reset`), so they're testable without TanStack DB; client mutations throw. `@tanstack/db` is a **type-only optional peer dependency** — the subpath only `import type`s `CollectionConfig`, nothing imports it at runtime, so the widget entry stays free of it. History/ordering/dedup stay the transport's contract. **Migrating the widget itself onto these collections is optional (it's already reactive) and would need the Vite plugin to bundle the widget — the dev panel's raw single-file ESM serving can't load `@tanstack/db`'s multi-file dep tree.**
