...
- `collections/` — read-only TanStack DB read model over the run stream, subpath export `./collections`. `createRunSource(transport)` wraps the transport as one shared fan-out stream (buffers the current run for late-attaching collections, resets on `run:start`); `runCollectionOptions`/`createRunCollection` point a projection at a source; `createRunModel(transport)` returns `{ source, scenes, assertions, close }` — many tables, one connection. Projections (`scenesProjection`, `assertionsProjection`) are the sole writers, folding `RunEvent`s into a tiny `RowOp` vocabulary (`insert`/`update`/`delete`/`reset`), so they're testable without TanStack DB; client mutations throw. `@tanstack/db` is an **optional peer dependency** — only the `./collections` subpath imports it (runtime `createCollection`), so the widget entry stays free of it. History/ordering/dedup stay the transport's contract. **The widget itself still runs on `store.ts`; migrating it onto these collections is a follow-up that also needs the Vite plugin to bundle the widget (the dev panel's raw single-file ESM serving can't load `@tanstack/db`'s multi-file dep tree).**
